...
### Key Features
* Adds support for running workflows on a k8s (Kubernetes) cluster. You can now register a k8s integration
    from the UI by providing the kubeconfig file that points to the cluster, and publish workflows
    to run on Kubernetes by modifying the `config` argument in the SDK's `publish_flow` API. 
* Enables using GCS (Google Cloud Storage) as Aqueduct's metadata store. You can register GCS as a storage
    integration from the UI and let Aqueduct store metadata in GCS.

### Enhancements
* Adds support for editing the config of existing integrations from the UI.
* Adds support for deleting integrations from the UI.
* When deleting a workflow, you can optionally let Aqueduct delete data generated by this workflow
    that's written to storage integrations.
