import pytest
from aqueduct_executor.operators.connectors.tabular import dataframe, snowflake
from aqueduct_executor.operators.connectors.tests import conf, utils

_TABLE = "test_snowflake"


@pytest.mark.skipif(conf.SKIP_SNOWFLAKE, reason="Skip Snowflake Flag Set")
class TestSnowflake:
    @classmethod
    def setup_class(cls):
        # Setup connector
        config = conf.SNOWFLAKE_CONF
        conn = snowflake.SnowflakeConnector(config)
        cls.conn = conn
    ...
    @pytest.mark.dependency(depends=["TestSnowflake::test_load"])
