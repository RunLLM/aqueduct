...
RUN conda init bash
RUN conda create -n py37_env python=3.7
RUN echo "conda activate py37_env" >> ~/.bashrc

RUN	wget --quiet https://awscli.amazonaws.com/awscli-exe-linux-x86_64.zip -O awscliv2.zip && \
    unzip awscliv2.zip && ./aws/install 

RUN conda activate py37_env
RUN pip install conda-pack aqueduct-ml==0.2.10

COPY ./spark/create-conda-env.sh /

ENV PYTHONUNBUFFERED 1

CMD ["conda", "run", "-n", "py37_env", "/bin/bash", "/create-conda-env.sh"]