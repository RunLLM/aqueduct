...
func (j *lambdaJobManager) Launch(ctx context.Context, name string, spec Spec) error {
	...
	log.Info("Encoded Spec")
	log.Info(encodedSpec)
...
func (j *lambdaJobManager) Poll(ctx context.Context, name string, metadataPath string, storageConfig *shared.StorageConfig) (shared.ExecutionStatus, error) {
	if workflow_utils.ObjectExistsInStorage(ctx, storageConfig, metadataPath) {
		return shared.LambdaCompletedExecutionStatus, nil
	} else {
		return shared.PendingExecutionStatus, ErrJobNotExist
	}
...
func mapIntegrationServiceToLambdaFunction(service integration.Service) (string, error) {
	...
	// 	return lambda_utils.MySqlLambdaFunction, nil
	// case integration.SqlServer:
	// 	return lambda_utils.SqlServerLambdaFunction, nil
