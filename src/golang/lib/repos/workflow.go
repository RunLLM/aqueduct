...
type workflowWriter interface {
	...
	// RemoveNotificationFromSettings removes `notificationResourceID` from notification_settings
	// field when possible.
	// If the ID does not appear in any notification_settings field,
	// the API simply return without error.
	RemoveNotificationFromSettings(ctx context.Context, notificationResourceID uuid.UUID, DB database.Database) error
}
