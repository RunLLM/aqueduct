...
func (ts *TestSuite) TestOperatorResult_GetStatusByDAGResultAndArtifactBatch() {
	...
	operatorIdA := uuid.New()
	expectedOperatorResultsA := ts.seedOperatorResultForDAGAndOperator(3, dagResultIdA, operatorIdA)
	
	dagResultIdB := uuid.New()
	operatorIdB := uuid.New()
	expectedOperatorResultsB := ts.seedOperatorResultForDAGAndOperator(3, dagResultIdB, operatorIdB)
	
	dagResultIdC := uuid.New()
	operatorIdC := uuid.New()
	_ := ts.seedOperatorResultForDAGAndOperator(3, dagResultIdC, operatorIdC)

	expectedOperatorResultsC := ts.seedOperatorResultForDAGAndOperator(3, dagResultIdA, operatorIdB)
	...
	actualOperatorResultStatuses, err := ts.operatorResult.GetStatusByDAGResultAndArtifactBatch(ts.ctx, []uuid.UUID{dagResultIdA, dagResultIdB}, []uuid.UUID{operatorIdA, operatorIdB}, ts.DB)
	require.Nil(ts.T(), err)
	requireDeepEqualOperatorResultStatuses(ts.T(), expectedOperatorResultStatuses, actualOperatorResultStatuses)
