...
type WorkflowDag interface {
	// WorkflowDag tracks the life cycle of ONE dag execution.
	ID() uuid.UUID
	UserID() uuid.UUID
	ResultID() uuid.UUID
	Name() string
	...
	// Operators that generate the input artifacts of a given operator.
	OperatorParents(operator.Operator) ([]operator.Operator, error)
...
type workflowDagImpl struct {
	...
	operators map[uuid.UUID]operator.Operator
	artifacts map[uuid.UUID]artifact.Artifact
	// Operator -> Artifacts generated as outputs.
	opToOutputArtifacts map[uuid.UUID][]uuid.UUID
	// Operator -> Artifacts that are used as inputs.
	opToInputArtifacts map[uuid.UUID][]uuid.UUID
	// Artifact -> Operators that use artifact as input. Can be many operators that use same artifact as input.
	artifactToOutputOps map[uuid.UUID][]uuid.UUID
	// Artifact -> Operator that generated artifact as output. An artifact can only be generated by one operator.
	artifactToInputOp map[uuid.UUID]uuid.UUID
}
...
func NewWorkflowDag(
	...
	return &workflowDagImpl{
		...
		artifactToOutputOps: artifactIDToOpIDs,
		artifactToInputOp:   artifactIDToInputOpID,
	}, nil
}
...
func (w *workflowDagImpl) OperatorsOnArtifact(artifact artifact.Artifact) ([]operator.Operator, error) {
	opIDs, ok := w.artifactToOutputOps[artifact.ID()]
	if !ok {
...
func (w *workflowDagImpl) OperatorParents(op operator.Operator) ([]operator.Operator, error) {
	inputArtifactIDs, ok := w.opToInputArtifacts[op.ID()]
	if !ok {
		return nil, errors.Newf("Unable to find operator %s (%s) on dag.", op.ID(), op.Name())
	}

	// Since each artifact can only be generated by one operator, the size of the input operators
	// and the size of the input artifacts are the same.
	ops := make([]operator.Operator, 0, len(inputArtifactIDs))
	for _, artifactID := range inputArtifactIDs {
		// Get ID of operator that produced artifact.
		parentOperatorID, ok := w.artifactToInputOp[artifactID]
		if !ok {
			return nil, errors.Newf("Unable to find operator %s (%s) on dag.", op.ID(), op.Name())
		}
		// Get operator from ID.
		parentOperator, ok := w.operators[parentOperatorID]
		if !ok {
			return nil, errors.Newf("Unable to find operator %s (%s) on dag.", op.ID(), op.Name())
		}
		ops = append(ops, parentOperator)
	}
	return ops, nil
}
