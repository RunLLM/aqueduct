import { archiveNotification, listNotifications, Notification } from '@aqueducthq/common/src/utils/notifications';
...
export const handleArchiveAllNotifications = createAsyncThunk(
    ...
    async (args: { user: UserProfile; notifications: Notification[] }) => {
        ...
        await Promise.all(notifications.map((notification) => archiveNotification(user, notification.id)));
...
export const notificationsSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(handleFetchNotifications.fulfilled, (state, { payload }) => {
        ...
        builder.addCase(handleArchiveAllNotifications.rejected, (state, { payload }) => {
            const errorMessage = payload as string;
            state.errorMessage = errorMessage;
        });
        ...
        builder.addCase(handleArchiveNotification.rejected, (state, { payload }) => {
            const errorMessage = payload as string;
            state.loading = false;
            state.errorMessage = errorMessage;
        });
