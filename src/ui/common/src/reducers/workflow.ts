...
export const handleGetWorkflow = createAsyncThunk<GetWorkflowResponse, { apiKey: string; workflowId: string }>(
    'workflowReducer/get',
    async (
        args: {
            apiKey: string;
        ...
        const res = await fetch(`${httpProtocol}://${apiAddress}/workflow/${workflowId}`, {
            method: 'GET',
            headers: {
                'api-key': apiKey,
            },
...
export const workflowSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder.addCase(handleGetOperatorResults.pending, (state, action) => {
            ...
            state.operatorResults[operatorId] = { loadingStatus: { loading: LoadingStatusEnum.Loading, err: '' } };
        });
        ...
        builder.addCase(handleGetArtifactResults.pending, (state, action) => {
            ...
            state.artifactResults[artifactId] = { loadingStatus: { loading: LoadingStatusEnum.Loading, err: '' } };
        });
        ...
        builder.addCase(handleGetWorkflow.fulfilled, (state, { payload }: PayloadAction<GetWorkflowResponse>) => {
        ...
        builder.addCase(handleGetWorkflow.rejected, (state, { payload }) => {
            state.loadingStatus = { loading: LoadingStatusEnum.Failed, err: payload as string };
        });
    },
});
