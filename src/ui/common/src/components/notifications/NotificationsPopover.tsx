...
import { Tabs, Tab } from '../primitives/Tabs.styles';
...
export const NotificationsPopover: React.FC<NotificationsPopoverProps> = ({
    ...
    const notificationsState = useSelector((state: RootState) => state.notificationsReducer);
                ...
                {filteredNotifications.length > 0 ? (
                    <List>
                        {filteredNotifications.map((notification, _) => {
                            return (
                                <NotificationListItem user={user} key={notification.id} notification={notification} />
                            );
                        })}
                    </List>
                ) : (
