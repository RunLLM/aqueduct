...
const DataPreviewer: React.FC<Props> = ({ previewData, error }) => {
    ...
    if (error) {
        errorMessage = (
            <LogBlock logText={error.trim()} level={LogLevel.Error} title="An error occurred during execution" />
        );
        ...
        errorMessage = (
            <Alert severity="warning">
                <AlertTitle> An upstream operator failed, ending the workflow execution. </AlertTitle>
            </Alert>
        );
    }
