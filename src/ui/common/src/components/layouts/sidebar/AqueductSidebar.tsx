...
import {CollapsedStatusBarWidthInPx, StatusBarWidthInPx} from "../../workflows/StatusBar";
import {
...
type Props = {
    ...
    children: ReactElement<any, any> | Component<{}, any, any>
};
 ...
 */
export const getBottomSideSheetWidth = (workflowStatusBarOpen: boolean, baseWidth = '100%'): string | string[] => {
    return `calc(${baseWidth} - ${MenuSidebarWidth} - ${2 * BottomSidebarMarginInPx}px - ${getBottomSidesheetOffset(
        workflowStatusBarOpen,
    )})`;
};
 ...
 */
export const getBottomSidesheetOffset = (workflowStatusBarOpen: boolean): string => {
...
export const AqueductSidebar: React.FC<Props> = ({
    ...
    const bottomSideSheetWidth = getBottomSideSheetWidth(openSideSheetState.workflowStatusBarOpen);
    const bottomSideSheetOffset = getBottomSidesheetOffset(openSideSheetState.workflowStatusBarOpen);
            ...
            <>
                <Box
                    sx={{
                        display: 'flex',
                        alignItems: 'center',
                        height: position === SidebarPosition.bottom ? BottomSidebarHeaderHeightInPx : '',
                        backgroundColor: theme.palette.gray['100'],
                    }}
                    py={1}
                >
