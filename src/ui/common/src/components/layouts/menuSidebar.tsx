import { faBell, faCircleUser, faDatabase, faMessage, faPlug, faShareNodes } from '@fortawesome/free-solid-svg-icons';
...
const SidebarButton: React.FC<SidebarButtonProps> = ({ icon, text, numUpdates = 0, selected = false }) => {
    return (
        <Button
            sx={{
                my: 1,
            ...
            >
                ...
                {!!numUpdates && (
                    <Box className={styles['notification-alert']}>
                        <Typography variant="body2" sx={{ fontSize: '12px', fontWeight: 'light', color: 'white' }}>
                            {numUpdates}
                        </Typography>
                    </Box>
                )}
...
const MenuSidebar: React.FC<{ user: UserProfile }> = ({ user }) => {
    ...
    const avatar = user.picture ? (
        <Avatar className={styles['user-avatar']} sx={{ width: '24px', height: '24px' }} src={user.picture} />
    ) : (
        <Avatar className={styles['user-avatar']} sx={{ width: '24px', height: '24px' }}>
            ...
            <Box className={styles['menu-sidebar-links']}>
                <Box className={styles['menu-sidebar-links-wrapper']}>
                    <Box className={styles['menu-sidebar-link']} onClick={handleClick}>
                        <SidebarButton
                            icon={<FontAwesomeIcon className={styles['menu-sidebar-icon']} icon={faBell} />}
                            text="Notifications"
                    ...
                    <Link href="/workflows" className={styles['menu-sidebar-link']} underline="none">
                        <SidebarButton
                            icon={<FontAwesomeIcon className={styles['menu-sidebar-icon']} icon={faShareNodes} />}
                            text="Workflows"
                            selected={currentPage === '/workflows'}
                        />
                    </Link>

                    <Link href="/integrations" className={styles['menu-sidebar-link']} underline="none">
                        <SidebarButton
                            icon={<FontAwesomeIcon className={styles['menu-sidebar-icon']} icon={faPlug} />}
                            text="Integrations"
                            selected={currentPage === '/integrations'}
                        />
                    </Link>

                    <Link href="/data" className={styles['menu-sidebar-link']} underline="none">
                        <SidebarButton
                            icon={<FontAwesomeIcon className={styles['menu-sidebar-icon']} icon={faDatabase} />}
                            text="Data"
                ...
                <Box sx={{ width: '100%' }}>
                    ...
                    <Box sx={{ my: 2 }}>
                        <Link href="mailto:support@aqueducthq.com" underline="none">
                            <SidebarButton
                                icon={<FontAwesomeIcon className={styles['menu-sidebar-icon']} icon={faMessage} />}
                                text="Report Issue"
                            />
                        </Link>
                    </Box>
