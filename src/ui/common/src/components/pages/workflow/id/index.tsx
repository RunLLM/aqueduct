...
import {
    AqueductSidebar,
    BottomSidebarHeaderHeightInPx, BottomSidebarHeightInPx,
    getBottomSideSheetWidth,
    SidebarPosition
} from "../../../layouts/sidebar/AqueductSidebar";
import WorkflowHeader from "../../../workflows/workflowHeader";
import {setAllSideSheetState, setBottomSideSheetOpenState} from "../../../../reducers/openSideSheet";
...
import {NodeType, resetSelectedNode} from "../../../../reducers/nodeSelection";
...
const WorkflowPage: React.FC<WorkflowPageProps> = ({ user, workflowId }) => {
    ...
    const currentNode = useSelector((state: RootState) => state.nodeSelectionReducer.selected);
    const workflow = useSelector((state: RootState) => state.workflowReducer);
    const [dagLayoutElements, setDagLayoutElements] = useState({ nodes: [], edges: [] });
    ...
    const artifactResult = useSelector((state: RootState) => state.workflowReducer.artifactResults[currentNode.id]);
    ...
    const updateLayout = () => {
        if (workflow.loadingStatus.loading === LoadingStatusEnum.Succeeded && !!selectedDag) {
            asyncSetDagLayoutElements(selectedDag.operators, selectedDag.artifacts, user.apiKey);
        }
    };
    ...
    const contentWidth = getBottomSideSheetWidth(openSideSheetState.workflowStatusBarOpen, fullWindowWidth);
    ...
    const getNodeActionButton = () => {
        if (currentNode.type === NodeType.TableArtifact) {
            ...
            return (
                <Button onClick={() => exportCsv(artifactResultData, getNodeLabel().replace(' ', '_'))}>
                    Export CSV
                </Button>
            );
        }
    ...
    return (
        <DefaultLayout user={user}>
            ...
            <Box sx={{ display: 'flex', width: contentWidth, height: '100%', flexDirection: 'column' }}>
                {workflow.selectedDag && <WorkflowHeader user={user} workflowDag={workflow.selectedDag} />}
