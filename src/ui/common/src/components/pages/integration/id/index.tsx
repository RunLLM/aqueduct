...
import { handleLoadIntegrationTable, tableKeyFn } from '../../../../reducers/integrationTableData';
...
const IntegrationDetailsPage: React.FC<IntegrationDetailsPageProps> = ({ user, integrationId }) => {
    ...
    useEffect(() => {
        ...
        dispatch(handleLoadIntegrationTables({ apiKey: user.apiKey, integrationId: integrationId }));
    }, []);

    const integrations = useSelector((state: RootState) => state.integrationsReducer.integrations);
    const integrationTables = useSelector((state: RootState) => state.integrationTablesReducer.integrationTables);
    const tableListStatus = useSelector((state: RootState) => state.integrationTablesReducer.thunkState);

    useEffect(() => {
        dispatch(handleLoadIntegrationTable({ apiKey: user.apiKey, integrationId: integrationId, table: table }));
    }, [table]);

    const tableKey = tableKeyFn(table);
    const [tableDataStatus, retrievedTableData] = useSelector((state: RootState) => {
    ...
    if (table !== '' && retrievedTableData && tableDataStatus === ExecutionStatus.Succeeded) {
    ...
    return (
        <DefaultLayout>
            <Head>
                <title> Integration Details: {selectedIntegration.name} | Spiral Labs </title>
            </Head>
                    ...
                    <Box sx={{ mt: 3 }}>
                        ...
                        {table !== '' && tableDataStatus === ExecutionStatus.Failed && (
                            <Alert style={{ marginTop: '10px' }} severity="error">
                                Table <b>{table}</b> failed to load. Try refreshing the page. <br />
                                Error: {retrievedTableData}
                            </Alert>
                        )}
                        {table !== '' && tableDataStatus === ExecutionStatus.Succeeded && retrievedTableData !== '' && (
