...
const WorkflowHeader: React.FC<Props> = ({ user, workflowDag }) => {
    ...
    const successMessage = 'Successfully triggered a manual update for this worfklow!';
    const [errorMessage, setErrorMessage] = useState('Unable to update this worfklow.');
    ...
    const triggerWorkflowRun = () => {
        ...
        fetch(`${httpProtocol}://${apiAddress}/workflow/${workflowDag.workflow_id}/refresh`, {
            method: 'POST',
            headers: {
                'api-key': user.apiKey,
            },
    ...
    const runWorkflowDialog = (
        <Dialog open={showRunWorkflowDialog} onClose={() => setShowRunWorkflowDialog(false)}>
            <DialogTitle>Trigger a Workflow Run?</DialogTitle>
            <DialogContent>
                Do you want to trigger a run of <code>{name}</code> immediately?
            </DialogContent>
            <DialogActions>
                <Button color="secondary" onClick={() => setShowRunWorkflowDialog(false)}>
                    Cancel
                </Button>
    ...
    return (
        <Box>
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
                ...
                <Box sx={{ ml: 2 }}>
                    <Button variant="outlined" color="primary" onClick={() => setShowSettings(true)} sx={{ py: 0 }}>
            ...
            {description && (
                <Typography variant="body1">
                    <ReactMarkdown className={style.reactMarkdown}>{description}</ReactMarkdown>
                </Typography>
            )}
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', my: 1 }}>
                {workflow.dagResults && workflow.dagResults.length > 0 && <VersionSelector />}
