...
const PeriodicScheduleSelector: React.FC<PeriodicScheduleSelectorProps> = ({ cronString, setSchedule }) => {
    ...
    return (
        <Box sx={{ display: 'flex' }}>
            <FormControl size="small" sx={{ mr: 1 }}>
                <Select value={timeUnit} onChange={(e) => setTimeUnit(e.target.value as PeriodUnit)}>
...
const WorkflowSettings: React.FC<WorkflowSettingsProps> = ({ user, workflowDag, open, onClose }) => {
    const { httpProtocol, apiAddress } = useAqueductConsts();
    const router = useRouter();

    const [name, setName] = useState(workflowDag.metadata?.name);
    const [description, setDescription] = useState(workflowDag.metadata?.description);
    const [triggerType, setTriggerType] = useState(workflowDag.metadata.schedule.trigger);
    const [schedule, setSchedule] = useState(workflowDag.metadata.schedule.cron_schedule);
    ...
    const settingsChanged =
        ...
        (triggerType === WorkflowUpdateTrigger.Periodic && schedule !== workflowDag.metadata.schedule.cron_schedule) || // The schedule type is still periodic but the schedule itself has changed.
    ...
    const scheduleSelector = (
        <Box sx={{ my: 2 }}>
            <RadioGroup
                onChange={(e) => setTriggerType(e.target.value as WorkflowUpdateTrigger)}
                value={triggerType}
                sx={{ width: '200px' }}
            >
                {triggerOptions.map(({ label, value }) => {
                    return (
                        <FormControlLabel
                            ...
                            key={value}
                            sx={{ [`& .${formControlLabelClasses.label}`]: { fontSize: '14px' } }}
                        />
                    );
                })}
            </RadioGroup>
            ...
            {triggerType === WorkflowUpdateTrigger.Periodic && (
                <>
                    <PeriodicScheduleSelector setSchedule={setSchedule} cronString={schedule} />
                    <FormControlLabel
                        sx={{ mt: 1, ml: 0 }}
                        label="Pause Workflow"
                        control={<Switch size="small" onChange={() => setPaused(!paused)} checked={paused} />}
                    />
                </>
            )}
        </Box>
    ...
    if (
        workflowDag.metadata?.schedule?.trigger === WorkflowUpdateTrigger.Periodic &&
        !workflowDag.metadata?.schedule?.paused
    ) {
        const nextUpdateTime = getNextUpdateTime(workflowDag.metadata?.schedule?.cron_schedule);
        nextUpdateComponent = (
            <Box sx={{ fontSize: '10px' }}>
                <Typography variant="body2">
                    <strong> Next Workflow Run: </strong> {nextUpdateTime.toDate().toLocaleString()}{' '}
                </Typography>
            </Box>
        );
    }
    ...
    const handleDeleteWorkflow = (event) => {
        ...
        fetch(`${httpProtocol}://${apiAddress}/workflow/${workflowDag.workflow_id}/delete`, {
            method: 'POST',
            headers: {
                'api-key': user.apiKey,
            },
            ...
            res.json().then((body) => {
                ...
                if (res.ok) {
                    ...
                    setDeleteMessage('Successfully deleted your workflow. Redirecting you to the workflows page...');
                    setShowDeleteMessage(true);
                    router.push('/workflows');
                } else {
                    setDeleteSucceeded(false);
                    setDeleteMessage(`We were unable to delete your workflow: ${body.error}`);
                    setShowDeleteMessage(true);
                    setDeleteValidation('');
                }
            });
    ...
    const updateSettings = (event) => {
        ...
        const changes = {
            ...
            description: name === workflowDag.metadata?.description ? '' : description,
            schedule: {
                trigger: triggerType, // We always set the trigger type to be safe because it's stored as a single JSON blob.
                cron_schedule: triggerType === WorkflowUpdateTrigger.Periodic ? schedule : '', // Always set the schedule if the update type is periodic.
                paused, // Set whatever value of paused was set, which will be the previous value if it's not modified.
            },
        };

        fetch(`${httpProtocol}://${apiAddress}/workflow/${workflowDag.workflow_id}/edit`, {
            method: 'POST',
            headers: {
                'api-key': user.apiKey,
            },
            ...
            res.json().then((body) => {
                ...
                if (res.ok) {
                    ...
                    setUpdateMessage(`There was an unexpected error while updating your workflow: ${body.error}`);
                }
        ...
        <Dialog open={showDeleteDialog} onClose={() => setShowDeleteDialog(false)}>
            <DialogTitle>
                <Typography variant="h5">
                    ...
                    Delete <span style={{ fontFamily: 'Monospace' }}>{workflowDag.metadata?.name}</span>?{' '}
                </Typography>
            </DialogTitle>

            <DialogContent>
                <Typography variant="body1">
                    Are you sure you want to delete <span style={{ fontFamily: 'Monospace' }}>{name}</span>? This action
                    is not reversible, and all data associated with your workflow will be deleted.
                </Typography>

                <Box sx={{ my: 2 }}>
                    <Typography variant="body1">Type the name of your workflow below to confirm deletion:</Typography>
                </Box>
            ...
            <DialogActions>
                <Button variant="outlined" color="secondary" onClick={() => setShowDeleteDialog(false)}>
                    Cancel
                </Button>
                <LoadingButton
                    variant="contained"
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                        <Box sx={{ flex: 1 }}>
                            <Typography variant="h5">
                                {' '}
                                {/* We don't use the `name` state here because it will udpate when the user is mid-changes, which is awkward. */}
                                <span style={{ fontFamily: 'Monospace' }}>{workflowDag.metadata?.name}</span> Settings{' '}
                            </Typography>
                        </Box>

                        <FontAwesomeIcon icon={faXmark} onClick={onClose} style={{ cursor: 'pointer' }} />
                    </Box>
                </DialogTitle>
