...
import {NodeType, OperatorTypeToNodeTypeMap, selectNode} from "../../reducers/nodeSelection";
import {
...
const ActiveWorkflowStatusTab: React.FC<ActiveWorkflowStatusTabProps> = ({ activeWorkflowStatusTab, listItems }) => {
    ...
    if (activeWorkflowStatusTab === WorkflowStatusTabs.Collapsed || !openSideSheetState.workflowStatusBarOpen) {
        return null;
    }
...
export const WorkflowStatusBar: React.FC<WorkflowStatusBarProps> = ({ user }) => {
    const dispatch: AppDispatch = useDispatch();
    const workflow = useSelector((state: RootState) => state.workflowReducer);
    const selectedDag = workflow.selectedDag;
    const artifacts: { [id: string]: Artifact } = workflow.selectedDag?.artifacts ?? {};
    const operators: { [id: string]: Operator } = workflow.selectedDag?.operators ?? {};

    const [activeWorkflowStatusTab, setActiveWorkflowStatusTab] = useState(WorkflowStatusTabs.Errors);
    ...
    const [workflowStatusItems, setWorkflowStatusItems] = useState<WorkflowStatusItem[]>([]);
    ...
    useEffect(() => {
        const filteredErrors: WorkflowStatusItem[] = workflowStatusItems.filter((workflowStatusItem) => {
            return workflowStatusItem.level === WorkflowStatusTabs.Errors;
        });

        const filteredWarnings: WorkflowStatusItem[] = workflowStatusItems.filter((workflowStatusItem) => {
            return workflowStatusItem.level === WorkflowStatusTabs.Warnings;
        });

        const filteredLogs: WorkflowStatusItem[] = workflowStatusItems.filter((workflowStatusItem) => {
            return workflowStatusItem.level === WorkflowStatusTabs.Logs;
        });

        const filteredChecks: WorkflowStatusItem[] = workflowStatusItems.filter((workflowStatusItem) => {
    ...
    const normalizeWorkflowStatusItems = () => {
        ...
        Object.keys(artifacts).map(async (artifactId) => {
            const artifactName = artifacts[artifactId].name ? artifacts[artifactId].name : 'Artifact';
            const artifactResult: ArtifactResult = workflow.artifactResults[artifactId];
        ...
        Object.keys(operators).map(async (operatorId) => {
            const operatorName = operators[operatorId].name ? operators[operatorId].name : 'Operator';
            const operatorResult: OperatorResult = workflow.operatorResults[operatorId];
            ...
            const newWorkflowStatusItem: WorkflowStatusItem = {
                ...
                message: '',
                nodeId: operatorId,
                type: OperatorTypeToNodeTypeMap[operators[operatorId].spec.type].toString(),
            };

            if (operatorResult.result?.status === ExecutionStatus.Failed) {
                // add to the errors array.
                newWorkflowStatusItem.level = WorkflowStatusTabs.Errors;
                if (operatorResult.result.error && operatorResult.result.error.length > 0) {
        ...
        return getUniqueListBy<WorkflowStatusItem>(normalizedWorkflowStatusItems, 'id');
    };
        ...
        >
            ...
            >
                <Box sx={{ cursor: 'pointer', my: 2, mx: collapsed ? 0 : 1 }}>
                    {collapsed ? (
                        <FontAwesomeIcon icon={faChevronLeft} onClick={expandWorkflowStatusbar} />
                    ) : (
                        <FontAwesomeIcon icon={faChevronRight} onClick={collapseWorkflowStatusBar} />
                    )}
                </Box>
            ...
            <ActiveWorkflowStatusTab activeWorkflowStatusTab={activeWorkflowStatusTab} listItems={listItems} />
        </Box>
    );
};
