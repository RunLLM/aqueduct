...
export const Node: React.FC<Props> = ({ data, defaultLabel, isConnectable, icon }) => {
    ...
    const workflowState = useSelector((state: RootState) => state.workflowReducer);
    ...
    if (data.nodeType === ReactflowNodeType.Operator) {
        ...
        if (workflowState.selectedDag.artifacts[data.nodeId]?.spec.type === ArtifactType.Bool) {
    ...
    return (
        ...
        >
            ...
            <Handle
                type="source"
                id="db-source-id"
                style={{ background: theme.palette.darkGray as string, border: theme.palette.darkGray as string }}
