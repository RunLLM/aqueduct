        ...
        <Card>
            ...
            <Box sx={{ flex: '1' }}>
                ...
                {workflow.description && (
                    <Box my={1}>
                        <Box sx={{ maxHeight: '80px', overflowY: 'hidden', textOverflow: 'ellipsis', mt: 1 }}>
                            <ReactMarkdown className={style.reactMarkdown}>{workflow.description}</ReactMarkdown>
                        </Box>
                    </Box>
                )}
            </Box>
    ...
    if (workflow['last_run_at']) {
        ...
        return (
            <Box onClick={() => setShowInfoToast(true)}>
                ...
                >
                    <Alert onClose={handleInfoToastClose} severity="success" sx={{ width: '100%' }}>
                        {toastMessage}
                    </Alert>
                </Snackbar>
            </Box>
