...
const OperatorResultsSideSheet: React.FC<Props> = ({ user, currentNode }) => {
    ...
    const logs = workflow.operatorResults[currentNode.id]?.result?.logs ?? {};
    const operatorError = workflow.operatorResults[currentNode.id]?.result?.error ?? '';
    const integrations = useSelector((state: RootState) => state.integrationsReducer);
    ...
    if (
        ...
        actions = (
            <>
                <Button
                    onClick={async () => {
                        const exportFunctionResult: ExportFunctionStatus = await handleExportFunction(
                            user,
                            currentNode.id,
                            functionFileName,
                        );
                ...
                >
                    <Alert
                        onClose={handleToastClose}
                        severity={toastMessage?.loadingStatus === 'success' ? 'success' : 'error'}
                        sx={{ width: '100%' }}
                    >
    ...
    if (operatorSpec.type === OperatorType.Extract || operatorSpec.type === OperatorType.Load) {
        ...
        switch (integration.service) {
            case 'Google Sheets':
                if (operatorSpec.type === OperatorType.Extract) {
                    ...
                    operatorParams.push(['Filepath', (spec.parameters as GoogleSheetsLoadParams).filepath]);
                }

                operatorParams.push(['Mode', (spec.parameters as GoogleSheetsLoadParams).save_mode]);
                ...
                if (operatorSpec.type === OperatorType.Extract) {
                    operatorParams.push([
                        ...
                        <Box sx={{ maxWidth: '600px' }} key="sql-query">
                            <code>{(spec.parameters as RelationalDBExtractParams).query}</code>
                        </Box>,
                    ]);
                } else {
                    operatorParams.push(['Table Name', (spec.parameters as RelationalDBLoadParams).table]);
                }
                operatorParams.push(['Mode', (spec.parameters as RelationalDBLoadParams).update_mode]);
        }
    ...
    const paramsView = operatorParams.map((parameter) => {
        ...
        return (
            ...
            // string value.
            <Box sx={{ display: valueIsString ? 'flex' : '', mb: 1 }} key={parameter[0]}>
    ...
    const tabs = [<Tab key="overview" label="Overview" />].concat(tableTabs.concat([<Tab key="logs" label="Logs" />]));
    ...
    const tableBoxes = artifactIds.map((artifactId, idx) => {
        ...
        return (
            ...
            >
                <DataPreviewer previewData={artifactResults[artifactId]} error={error} />
            </Box>
        );
    });
            ...
            <Box sx={{ height: tabContentHeight, p: 1 }}>
                <Box role="tabpanel" hidden={selectedIndex !== 0}>
                    ...
                    {operator.description && (
                        <ReactMarkdown className={style.reactMarkdown}>{operator.description}</ReactMarkdown>
                    )}
