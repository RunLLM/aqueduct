...
export const dataCardName = (dataPreviewInfo: DataPreviewInfo) =>
        ...
        return (
            <Link underline="none" color="inherit" href={`/workflow/${workflowId}`}>
                <Box sx={{ display: 'flex', flexDirection: 'column', width: '900px' }}>
                    ...
                    <WorkflowStatus status={dataPreviewInfo.versions[latestVersionUUID].status} />

                    <Typography variant="body1">
                        <strong>Saved at: </strong>
                        {new Date(dataPreviewInfo.versions[latestVersionUUID].timestamp * 1000).toLocaleString()}
                    </Typography>
...
export const IntegrationCard: React.FC<IntegrationProps> = ({ integration }) => {
    let serviceCard;
