...
export const IntegrationFileUploadField: React.FC<IntegrationFileUploadFieldProps> = ({
  label,
  description,
  required,
  file,
  ...
  if (file) {
      header = (
          <Box>
              ...
              <Button size="small" variant="outlined" component="span" onClick={onReset} sx={{ float: 'right' }}>
                  Choose File
              </Button>
          </Box>
      );
      ...
      const parsedColumns = parsedHeader.map((headerName) => {
          return { field: headerName, headerName: headerName, width: width * headerName.length };
      });
