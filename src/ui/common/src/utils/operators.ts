...
export async function exportFunction(user: UserProfile, operatorId: string): Promise<Blob> {
    const res = await fetch(`${httpProtocol}://${apiAddress}/function/${operatorId}/export`, {
        method: 'GET',
        headers: {
            'api-key': user.apiKey,
        },
