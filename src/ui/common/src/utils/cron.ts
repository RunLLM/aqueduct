...
function deconstructCronString(cronString: string): Schedule {
    ...
    const hourString = parsed.fields.hour[0] < 10 ? `0${parsed.fields.hour[0]}` : `${parsed.fields.hour[0]}`;
    const minuteString = parsed.fields.minute[0] < 10 ? `0${parsed.fields.minute[0]}` : `${parsed.fields.minute[0]}`;
...
function getNextUpdateTime(cronString: string | undefined): CronDate | undefined {
    if (!cronString) {
        return undefined;
    }
