...
export async function listNotifications(user: UserProfile): Promise<[Notification[], string]> {
    try {
...
export async function archiveNotification(user: UserProfile, id: string): Promise<string> {
    try {
        const res = await fetch(`${httpProtocol}://${apiAddress}/notifications/${id}/archive`, {
            method: 'POST',
            headers: { 'api-key': user.apiKey },
        });
