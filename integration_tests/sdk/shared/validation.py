...
"""These helpers are shared across both resource test suites."""


def fetch_and_validate_saved_object_identifier(
    data_resource: BaseResource, flow: Flow, artifact_id: uuid.UUID
) -> str:
    """Validates that the saved object exists according to the Flow API."""
    all_saved_objects = flow.list_saved_objects()[data_resource.name()]
