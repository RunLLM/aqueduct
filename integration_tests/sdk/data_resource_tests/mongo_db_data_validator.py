...
from sdk.data_resource_tests.relational_data_validator import RelationalDataValidator
...
class MongoDBDataValidator(RelationalDataValidator):
    ...
    _client: Client
    _resource: MongoDBResource

    def __init__(self, client: Client, resource: MongoDBResource):
        super(MongoDBDataValidator, self).__init__(client, resource)
    ...
    def check_saved_artifact_data(
        ...
        saved_data = self._resource.collection(saved_object_identifier).find({}).get()
