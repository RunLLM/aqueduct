...
class RelationalDataValidator:
    ...
    _client: Client
    _resource: RelationalDBResource

    def __init__(self, client: Client, resource: RelationalDBResource):
        self._client = client
    ...
    def check_saved_artifact_data(
        ...
        saved_data = self._resource.sql(query="SELECT * from %s" % saved_object_identifier).get()
