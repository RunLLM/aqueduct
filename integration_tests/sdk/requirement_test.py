...
from requirements.model import sentiment_prediction_using_transformers
...
def sentiment_prediction_with_valid_reqs_path(reviews: pd.DataFrame) -> pd.DataFrame:
    """This uses the requirements.txt in the requirements/ subdirectory to install transformers."""
    import transformers
    model = transformers.pipeline("sentiment-analysis")
    return reviews.join(pd.DataFrame(model(list(reviews["review"]))))
...
def test_default_requirements_installation(client):
    """
    This uses the decorated function in the requirements/ subdirectory, which already has a requirements.txt
