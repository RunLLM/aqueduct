...
class TestBackend:
    ...
    def test_endpoint_dag_results_get(self):
        ...
        resp = self.get_response(self.GET_DAG_RESULTS_TEMPLATE % flow_id,
            additional_headers={
            "order_by": "workflow_dag_result.status",
        }).json()
        ids = [result['id'] for result in resp]
        sorted_ids = sorted(ids, reverse=True)  # Descending order
        ...
        resp = self.get_response(self.GET_DAG_RESULTS_TEMPLATE % flow_id,
            additional_headers={
            "order_by": "workflow_dag_result.id",
            "limit": "1",
        }).json()

        workflow_id = [result['id'] for result in resp]
        assert len(workflow_id) == 1
        workflow_id = workflow_id[0]
        assert workflow_id == sorted_ids[0]
