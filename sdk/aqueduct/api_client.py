...
class APIClient:
    ...
    def list_integrations(self) -> Dict[str, IntegrationInfo]:
        url = self._construct_full_url(self.LIST_INTEGRATIONS_ROUTE)
        headers = utils.generate_auth_headers(self.api_key)
        resp = requests.get(url, headers=headers)
        utils.raise_errors(resp)
        if len(resp.json()) == 0:
    ...
    def list_github_repos(self) -> List[str]:
        url = self._construct_full_url(self.LIST_GITHUB_REPO_ROUTE)
        headers = utils.generate_auth_headers(self.api_key)
    ...
    def list_github_branches(self, repo_url: str) -> List[str]:
        url = self._construct_full_url(self.LIST_GITHUB_BRANCH_ROUTE)
        headers = utils.generate_auth_headers(self.api_key)
    ...
    def list_tables(self, limit: int) -> List[Tuple[str, str]]:
        url = self._construct_full_url(self.LIST_TABLES_ROUTE)
        headers = utils.generate_auth_headers(self.api_key)
    ...
    def preview(
        ...
        url = self._construct_full_url(self.PREVIEW_ROUTE)
    ...
    def register_workflow(
        ...
        url = self._construct_full_url(self.REGISTER_WORKFLOW_ROUTE)
        resp = requests.post(url, headers=headers, data=body, files=files)
        utils.raise_errors(resp)
    ...
    def refresh_workflow(
        ...
        headers = utils.generate_auth_headers(self.api_key)
        url = self._construct_full_url(self.REFRESH_WORKFLOW_ROUTE_TEMPLATE % flow_id)
    ...
    def delete_workflow(self, flow_id: str) -> None:
        headers = utils.generate_auth_headers(self.api_key)
        url = self._construct_full_url(self.DELETE_WORKFLOW_ROUTE_TEMPLATE % flow_id)
        response = requests.post(url, headers=headers)
        utils.raise_errors(response)

    def get_workflow(self, flow_id: str) -> GetWorkflowResponse:
        headers = utils.generate_auth_headers(self.api_key)
        url = self._construct_full_url(self.GET_WORKFLOW_ROUTE_TEMPLATE % flow_id)
        resp = requests.get(url, headers=headers)
        utils.raise_errors(resp)
    ...
    def list_workflows(self) -> List[ListWorkflowResponseEntry]:
        headers = utils.generate_auth_headers(self.api_key)
        url = self._construct_full_url(self.LIST_WORKFLOWS_ROUTE)
    ...
    def get_node_positions(
        ...
        """
        url = self._construct_full_url(self.NODE_POSITION_ROUTE)
        headers = {
            **utils.generate_auth_headers(self.api_key),
        }
    ...
    def export_serialized_function(self, operator: Operator) -> bytes:
        headers = utils.generate_auth_headers(self.api_key)
        operator_url = self._construct_full_url(self.EXPORT_FUNCTION_ROUTE % str(operator.id))
